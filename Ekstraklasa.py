...
def menu():
    ...
    choice = None
    while choice != 0:
        print(f'{typer.title()} - punkty: {punkty}')
        print('\nWybierz {} co chcesz zrobić?'.format(typer.title()))
        print(f'\nKolejka nr {kolejka}')
        print('\n1 - Zobacz zestaw par')
...
# zliczanie punktów wszystkich typerów, pobiera dane z bazy sql
def punktacja():
    typerp = 'kuba'
...
# zlicza punkty w poszczególnych kolejkach
def punktacjakolejka():
    ...
    for i in range(30):  # 30 kolejek
        print('')
        punkts = []  # zbiera sumy punktów do listy
        for a in scorer:
...
# pobiera daty dla danej kolejki z bazy sql
def terminy():
    ...
    for i in daty:
        data.append(i[0])
    return data  # jako lista dat dla danej kolejki
...
#  typowanie i edytowanie typów w pętli
def typowanieedit():
    typy_show()
    while True:
        try:
...
# pokazuje typy wszystkich graczy w danej kolejce
def typyall():
...
# wczytuje typy z bazy danych sql
def typy_load():
    ...
    for i in typy:
        typ.append(i[0])
    return typ  # jako lista typów dla danej kolejki


def typy_show():
    # WYSWIETLANIE TYPÓW DANEJ KOLEJKI
    typ = typy_load()  # wczytuje typey
...
# zczytuje wyniki z bazy danycb sql
def wyniki():
    cursor.execute(
    ...
    for i in wyniki:
        wynik.append(i[0])
    return wynik  # wyniki w liście dla meczów w danej kolejce
...
def wyniki90():
    ...
    wyniki = []  # multiple wyniki dla wszystkich kolejek
    ...
    a = 0
    b = 8
    results = []  # wyniki już dla poszczególnych kolejek podzielonych na listy
    for i in range(30):
        results.append(wyniki[a:b])
        a += 8
        b += 8

    term = []  # multiple daty dla wszsytkich kolejek w jednej liscie
    ...
    a = 0
    b = 8
    termin = []  # listy dat podzielonych na poszczególne kolejki
    for i in range(30):
        termin.append(term[a:b])
        a += 8
        b += 8

    for i in range(30):  # zapisywanie wyników ze strony do bazy danych sql
        result = results[i]
        data = termin[i]
        ...
        for a in range(8):
            ...
            dane = dane.replace(',', '')
            try:
                if dane[-1] in range(0, 10):  # jeżeli pobrana data kończy się cyfrą
...
# wczytanie danych dla typera, wartości globalne ze względu na fakt, iż po wgraniu są niezmiennej dla każdej fukcji
def wczytanie():
    global kolejka
    global mecze
    global data
    ...
    # zwraca nr kolejki oraz zestaw par, daty, wyniki i typy typera dla danej kolejki
...
def zestawpar():
    ...
    for i in pary:
        mecze.append(i[0])
    return mecze  # jako lista meczy dla danej kolejki
...
def zliczanie():
    ...
    for a in typery:
        ...
        for i in range(240):  # 30 kolejek po 8 meczy = 240
            typz = str(test[i][0])

...
# konwersja przypisuje pobrany miesiąc ze strony do numeru miesiąca i roku w celu ustalenia daty w module datetime
konwersja = {
...
def punktacjakolejka():
    ...
    for a in scorer:
        print('')
        print('{} punktował: '.format(a.title()))
        for i in range(30):
            ...
            pointy = sum(punkty)
            print('Kolejka nr {} - punkty: {}'.format(i+1, pointy))
    input('')
...
def typowanieedit():
    ...
    if i == 0:
        ...
        if len(kon) != 3:
            ...
            db.commit()
            exit = input('Chcesz typować inny mecz? T/N: ')
            exit = str(exit)
            if exit.lower() == 'n':
                clear()
                menu()
...
def zliczanie():
    ...
    cursor.execute('''SELECT typ FROM typowanie WHERE typer="kuba"''')
    test = cursor.fetchall()
    cursor.execute('''SELECT wynik  FROM typowanie WHERE typer="kuba"''')
